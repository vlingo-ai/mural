...
describe('MuralReconnectPolicy', () => {
  it('keeps retrying through a normal Wi-Fi reassociation', () => {
    ...
    expect(policy.nextRetryDelayInMs({ retryCount: 10, elapsedMs: 25_000 })).toBe(7_000);
    expect(policy.nextRetryDelayInMs({ retryCount: 20, elapsedMs: 32_000 })).toBe(7_000);
  });

  it('caps SDK retries at the same bounded window as the app', () => {
    const policy = new MuralReconnectPolicy();

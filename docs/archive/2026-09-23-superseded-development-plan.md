...
> 已归档：2026-09-23 被[最新开发基准](../web-ios-model-gateway-plan.md)取代。
> 以下保留旧计划与当时进度证据，不再决定开发顺序、部署或验收。
> “当前”“未部署”、普通话同步推进、Gateway Live、旧三端路径及报价均按历史语境理解；
> 不执行本文件中的部署/付费操作，不把旧测试结果外推到最新三端版本。
...
├──────────────┤                    │ auth / learning      │
│ vLingo iOS   │                    │ sessions / billing   │
└──────┬───────┘                    └──────┬────────┬───────┘
       │ LiveKit WebRTC                    │        │ Responses/tasks
       ▼                                  │        ▼
┌──────────────┐ room/control ┌────────────▼─┐  ┌────────────────────┐
│   LiveKit    │◀────────────▶│ Agent Worker │  │   Model Gateway    │
│ Cloud/Server │              │ GPT-Live     │  │ Responses / ASR /  │
└──────────────┘              └──────┬───────┘  │ Alignment routing  │
                                    │           └─────────┬──────────┘
                                    ▼                     ▼
                              OpenAI GPT-Live      OpenAI / third parties
```

Live 的默认链路：

1. Web 或 iOS 向 Mural API 创建产品会话。
2. Mural API 建立 LiveKit room、dispatch Agent Worker，并签发最小权限短期 token。
3. 客户端通过 LiveKit WebRTC 加入 room；不持有 OpenAI key。
4. Agent Worker 使用服务端 OpenAI key 连接 `gpt-live-1`，负责实时模型协议与委托。
5. Worker 向 Mural API 回传可信 usage、provider rejection、delegation 和 heartbeat；
   Mural API 负责额度、会话账本、强制关闭与结算。
6. 非实时 Responses、ASR 和 Alignment 由 Mural API 调用 Model Gateway；Model Gateway
   不拥有 Live capability，也不决定客户端是否显示实时会话。

未配置 LiveKit 时可显式使用 OpenAI WebRTC 直连 provider 做本地诊断回退；该路径不经过
Model Gateway，也不是 Phase 5.5B/C 的产品部署拓扑。
...
当前 Model Gateway `main` 是 Apple Silicon 上的 FastAPI + MLX 音频网关，已经实现
ASR、强制对齐、阅读诊断和流式转写。Phase 5.5B 将在独立分支补齐可移植的 Responses
provider/router；历史 Model Gateway Live 原型不进入最终架构。
...
### Phase 3（历史方案，已停止）：Model Gateway Live + sideband

此方案已被 Phase 5.5 的 LiveKit Agent Worker 架构取代，最终代码不保留
Model Gateway Live adapter。以下内容仅记录当时的验证目标，不是当前部署要求。
...
- 实现现有 `HostedResponsesTransport` 的 Gateway adapter；`LiveProvider` 由 Mural API
  在 LiveKit 和 OpenAI 直连之间选择，不经过 Model Gateway。
...
- 不删除现有 OpenAI WebRTC 本地诊断回退；不恢复已停止的 Gateway Live sideband。
...
Model Gateway Live adapter 原型已随架构收敛移除。当前边界固定为：Model Gateway 负责
Responses、ASR 和 Alignment；Mural API 负责产品 capability 与会话账本；Agent Worker
负责 GPT-Live provider。`MODEL_GATEWAY_URL` 与 `MODEL_GATEWAY_API_KEY` 仅影响非实时模型
路由，不参与 Live capability 或 Live provider 选择。
...
配置了 Gateway 时只有 Responses 走 Gateway；Live 由 LiveKit/Agent Worker 承载，缺省路径
仍保留显式的 OpenAI 本地诊断回退。TypeScript check、39 项聚焦测试和无数据库全量测试通过，
PostgreSQL 全量及本分支 GitHub checks 已通过。

Phase 4 的共享 Gateway client 已补齐：启动时验证公开 `/healthz` 且不发送 Gateway
credential；Responses 使用统一目标、鉴权和超时边界；连续三次 transport、timeout、

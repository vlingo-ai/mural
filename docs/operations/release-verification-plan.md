...
本地 Web 91/91 与类型检查 PASS，Linux 复验待办。复用规则：非空字符串不等于有效地址，
脱敏占位符、缺失值及冲突值均须显式覆盖；不得通过重跑或放宽断言覆盖失败。
部署前 DB/队列/Cloud 为 0，异机备份校验与解密/gzip PASS；非恢复演练，切换前重新检查空闲。
本轮尚未合并/部署/付费测试。
后续修复候选 `ab02af8` 的 [CI 36232128871](https://github.com/vlingo-ai/mural/actions/runs/36232128871)
已全部适用门禁 PASS：Linux 媒体三轮 24/24（零重试）、Web 91/91、E2E 3/3、
服务端与双镜像构建；本地构建/E2E 同样 PASS。上述失败保留，不以历史通过覆盖。
此处仅归集证据，文档提交的最新 CI 另验，仍未部署 B6。
...
历史投递卡住不能阻塞用量，存储失败不能留下半健康重放服务；积压检查涵盖两类队列；
随机回执代次不作为对话顺序，公平重试的提交顺序也不等于原始对话顺序，须独立验证。
实时回调、写入方切换、Web 补取及跨仓故障链路仍待完成，B6 不签结。

2026-09-26 B5 签结 / B6 启动（[记录](../../verification/2026-09-26-b6-history-delivery.md)）：
B5 最终候选 `70c515d` 的完整 CI `36228557251` PASS，真实媒体 24/24 零重试；PR #49 已合并为 `63a65d2`，仅测试变更无需部署。
B6 两仓已建立规范工作树并完成源码初查；本轮仅设计/文档检查，运行时测试 NOT_RUN，无部署/付费。
HISTORY-01 补充：持久 ACK 必须与事件身份/内容关联，浏览器与 Worker 双写须有统一身份；
cursor 测试须覆盖并发提交乱序，不能把序列分配顺序当作提交顺序。usage 合并队列不能直接用于逐条历史。
实现、测试及上线仍待后续，不将本轮设计记为已实现。
同日 B6 第一轮 API 候选：迁移 030 的会话级 cursor 分配、鉴权分页及重复事件内容冲突拒绝已实现于本地；
...
同日第七轮：[PR #49](https://github.com/vlingo-ai/mural/pull/49) 候选 `90188ce` 首次 Linux CI：
...
运行 `36225838179`：既有媒体 6 项 PASS，新丢包用例在 PID 1 命名空间读取处 EACCES，未插入规则。
修复只提权只读 readlink，并校验标识格式/差异，保留失败关闭；Linux 重跑待确认。
修复 `987cb1c`、运行 `36226258315`：Web PASS，媒体 **7/7 PASS（19.8 秒）**，含真实 UDP 上行丢包及撤销后恢复；
...
稳定性复查：`06228b4` 的完整运行 `36227249800` 服务端/部署检查通过，但媒体八项首次建连全失败；
...
固定回复匹配，输入 13/输出 7 token、搜索 0。估算供应商成本 USD 0.0000048，非官方账单对账。
未经过 Mural 用户账本，不代表其结算或其他路由验收；一次性执行标记保留，授权已用，不自动追加测试。
复用规则：真实 smoke 明确路径及计费覆盖范围，一次授权配一次性防重复标记；不得把 Gateway 成功扩大为端到端通过。
归档复查同时修正总计划中旧的“升级部署待办”和 B2→B3 下一步描述，后续为 B5→B6→B7；无新增运行时测试或付费授权。

模型升级须同时核对数据库费率版本约束、实际 provider/model 身份、公开价格元数据和账本预算边界。
保留旧快照，不通过重写费率或放宽约束使新请求通过；API 与逻辑路由配置须共同切换和回滚。

GPT-6 Luna 本地候选（2026-09-26，基线 `c956a05`）：[验证记录](../../verification/2026-09-26-gpt6-luna-upgrade.md)。
首轮 9 FAIL 已归因修复；最终 API 428 PASS/1 SKIP/0 FAIL，类型/构建 PASS。
跨仓 Worker 用例本轮未配；PR #46 `91a80ae` 的适用 CI（服务端、Web、部署、契约、扫描、汇总门禁）PASS，
原生重型检查按范围跳过。已复查，无新增规则变更；尚未合并、部署或真实模型验证，无付费调用。
部署准备：六项 DB 会话/辅助任务门禁计数均为 0；新加密备份 VPS/Mac 校验和一致，
Mac 解密与 gzip 校验 PASS，恢复演练 NOT_RUN；详见同一验证记录。切换前须重新核对空闲状态。
后续证据提交的密钥扫描误报回滚镜像标签；本地复现后限定文件及精确匹配例外，更新提交须重新通过 CI，不沿用旧提交的通过结论。
最终候选 `6f599e0` 的适用 CI 全部 PASS；PR #46 已合并为 `7979f865250a0bb95edf846e80eb77f114ae56c4`。
此为源码合并，不是上线验收；VPS 四路模型仍为旧版本，镜像构建及部署待办。
后续部署已执行：新 API 镜像/版本/平台核对、两组件候选配置解析与限定差异检查 PASS；
切换前 DB 阻断数 0，Gateway→API 健康等待 PASS，部署后均 healthy、重启 0，四路均为新模型。
公网健康及价格元数据 PASS；数据库容器内 SELECT 1 返回 1，启动后 API 26 行/Gateway 28 行日志的选定错误标记均为 0。
该 DB 检查不替代真实 API 数据库操作，日志扫描是有限标记检查；真实 provider 可用性与质量 NOT_RUN。
复用规则：同一发布目录内不同组件仍须显式选对应 env/Compose 和镜像覆盖文件，禁止全栈误启动。

恢复共享轨迹须分别校验产品 generation 与 Room epoch；Stop 后迟到事件不得重新 Active。
媒体权限异步结果须先检查 generation/closed，再赋给当前会话；过期结果须停止全部音轨，不能覆盖新会话媒体。
关闭 HTTP 成功不等于会话 closed；本地停音与服务端关闭确认须分开测试，失败/非终态不得由定时器伪装 idle。
产品截止须消耗请求在途时间，不能因 SDK 恢复或 Room 重建重新计时；客户端墙钟假设和服务端真正授权必须分开说明。
创建响应丢失不等于未准入；须按原幂等身份查证，不得直接创建第二个付费会话或将未知清成 idle。
原请求查询返回 null 只是当时未找到；没有原子取消凭据时不得当作取消成功，且客户端新查询接口须先部署 API 再 Web。
上线查询 smoke 须分别记录鉴权拒绝与登录态未知请求；owner 隔离、closed 分支的隔离测试不能冒充线上实测，历史页面成功不能替代新接口证据。
...
| 2026-09-26：B4 恢复门禁与关闭确认 | [第三轮](../../verification/2026-09-26-b4-recovery-protocol.md#第三轮恢复状态门禁与关闭确认) | Web 63/63、类型/构建、mock E2E 2/2 PASS；未部署/付费；复用现有服务端契约 | 状态查询失败不授权重入；Stop 立即停音，只有 closed 才 idle；仍缺完整控制截止与适配矩阵 |
| 2026-09-26：B4 信令与权限竞态 | [第二轮](../../verification/2026-09-26-b4-recovery-protocol.md#第二轮web-适配的两个有界修复) | Web 52/52、类型/构建、mock E2E 2/2 PASS；本地候选，未部署/付费；服务端未改 | signal-only 不自动重建 Room，既有截止不延长；过期媒体结果主动释放。状态查询/close 确认仍待办 |
